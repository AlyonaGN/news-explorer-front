...
function AppHeader({ onMenuClick, isMenuShown, isLoggedIn, isFontDark, onAuthClick, closeMenuOnclick, onSignOut }) {
            ...
            <header className={isMenuShown ? "app-header app-header_with-background" : "app-header"}>
                <p className="app-header__logo"> NewsExplorer </p>
                    <div className={isMenuShown ? "app-header__menu app-header__menu_popuped" : "app-header__menu"}>
                        <Navigation isUserLoggedIn={isLoggedIn} isMenuDisplayed={isMenuShown} isDark={isFontDark} onAuth={onAuthClick} closeMenu={closeMenuOnclick} signOut={onSignOut}/>
                    </div>
                { isMenuShown ? 
